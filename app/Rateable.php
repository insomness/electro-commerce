...
trait Rateable
{
    ...
    public function sumRatingWhere($rating)
    {
        return $this->ratings()->where('rating', $rating)->sum('rating');
    }
    ...
    public function ratingPercentByValue($rating, $max = 5)
    {
        $quantity = $this->ratings()->where('rating', $rating)->count();
        $total = $this->sumRatingWhere($rating);
